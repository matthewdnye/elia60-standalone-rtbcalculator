...
export const TaxBracketInfo: FC = () => {
  ...
  return (
    <div className="mt-4 bg-gray-50 rounded-lg p-4">
      <h3 className="text-lg font-medium text-gray-900 mb-4">2024 Tax Brackets</h3>
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-100">
            <tr>
              <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Rate</th>
              <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Single</th>
              <th className="px-4 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Married Filing Jointly</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-200">
            {taxBrackets.map((bracket, index) => (
              <tr key={index} className="hover:bg-gray-50">
                <td className="px-4 py-2 text-sm font-medium text-gray-900">{bracket.rate}</td>
                <td className="px-4 py-2 text-sm text-gray-500">{bracket.single}</td>
                <td className="px-4 py-2 text-sm text-gray-500">{bracket.married}</td>
              </tr>
            ))}
          </tbody>
        </table>
